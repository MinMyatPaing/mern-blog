...
export default function Comment({ comment, onClickLike, onEditComment, onDelete }) {
  ...
  useEffect(() => {
    const getUser = async () => {
      try {
        const res = await fetch(`/api/user/get-comment-user/${comment.userId}`);
      ...
      <div className="flex-1">
        <div className="flex items-center mb-1">
          <span className="font-bold mr-1 text-xs truncate">
            {user ? `@${user.username}` : "annoymous user "}
          </span>
